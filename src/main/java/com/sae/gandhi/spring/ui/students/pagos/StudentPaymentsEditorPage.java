...
public class StudentPaymentsEditorPage extends VerticalLayout implements HasUrlParameter<String> {
    ...
    @Autowired
    public StudentPaymentsEditorPage(AlumnosService alumnosService, CursosService cursosService, AlumnoPagoService alumnoPagoService){
	...
	public void setParameter(BeforeEvent event, @WildcardParameter String parameter) {
		...
		if(parameter != null && !parameter.equals("")){
			...
			try{
				...
				//Carga la información del alumno "Nombre", "Curso", "Saldo" a favor si existe y "Boton para regresar a la edicion del alumno"
			}catch(Exception e){
...
//        searchField.addValueChangeListener(e -> updateView());
	...
	public void addStudentData(){
        ...
        lbStudent.setText(alumnoVO.getAlumnoNombre()+" "+alumnoVO.getAlumnoApPaterno()+" "+alumnoVO.getAlumnoApMaterno());
	...
	private void addGridLayout(){
		...
		grid.addColumn(new ComponentRenderer<>(this::createConceptLabel)).setHeader("Concepto").setFlexGrow(4).setResizable(true);
		grid.addColumn(new NumberRenderer<>(AlumnoPagoVO::getAlumnoPagoMonto, NumberFormat.getCurrencyInstance())).setHeader("Monto").setFlexGrow(1);
		Column<AlumnoPagoVO> columnDate = grid.addColumn(new LocalDateRenderer<>(
				AlumnoPagoVO::getAlumnoPagoFechaLimite,DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM)))
				.setHeader("Fecha Límite").setFlexGrow(1);
		//Boton para editar
		Column<AlumnoPagoVO> editorColumn = grid.addComponentColumn(vo -> {
			if(vo.getEstatusId()==SaeEnums.Pago.ADEUDO.getStatusId() || vo.getEstatusId()==SaeEnums.Pago.PREPARADO.getStatusId()){
		...
		grid.addColumn(new NumberRenderer<>(AlumnoPagoVO::getAlumnoPagoPago, NumberFormat.getCurrencyInstance())).setHeader("Pago").setFlexGrow(1);
		grid.addColumn(new LocalDateRenderer<>(
				AlumnoPagoVO::getAlumnoPagoFechaPago,DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM))).setHeader("Fecha Pago").setFlexGrow(1);
		...
		editor.addSaveListener(
		        event -> {
		        	//System.out.println(event.getItem().getAlumnoPagoId() + " - " +event.getItem().getAlumnoPagoFechaLimite());
		        	alumnoPagoService.updateFecha(event.getItem());
		        	Notification.show("Se ha actualizado la fecha límite de pago a " + event.getItem().getAlumnoPagoFechaLimite());
		        }); 
	...
	private Label createConceptLabel(AlumnoPagoVO vo){
		...
		System.out.println("-----------"+vo.getAlumnoPagoFechaLimite().getMonth().name() + " " +vo.getAlumnoPagoFechaLimite().getYear() );
		StringBuilder sb = new StringBuilder(vo.getCursoNombre());
		sb.append(" ")
			.append(SaeEnums.Mes.getMes(vo.getAlumnoPagoFechaLimite().getMonth().getValue()))
